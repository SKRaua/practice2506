<template>
  <div class="common-layout">
          ...
          <el-row :gutter="30">
            ...
            <el-col :span="8">
              <el-form-item label="入住日期：">
                <el-date-picker value-format="YYYY-MM-DD" format="YYYY/MM/DD" v-model="dateChange" type="daterange"
    ...
    <!-- 床位调换对话框 -->
    <el-dialog v-model="exchangeData.dialogVisible" title="床位调换" width="60%" :before-close="exchangeClose" align-center
      draggable>
      <el-divider border-style="double" style="margin:0;" />
      <el-form label-position="right" label-width="auto" style="max-width:800px;margin:20px auto"
        ...
        <el-form-item label="新床号" prop="newBedId">
          <el-select style="width:200px" v-model="exchangeData.exchangeForm.newBedId" placeholder="请选择床位">
            <el-option ref="bedlist" v-for="item in exchangeData.bedList" :key="item.bedNo" :label="item.bedNo"
  ...
  methods: {
    ...
    del(id) {
      this.$confirm('此操作删除记录, 是否继续?', '提示', {
        ...
        removeBedDetails({ id: id }).then(res => {
          if (res.flag) {
    ...
    editSave(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          updateBedDetails(this.edit.bedDetalsForm).then(res => {
            if (res.flag) {
              this.$message.success(res.message);
    ...
    //api-查询床位详情信息(分页)
    getBedDetailsList() {
      listBedDetailsPage(this.queryParams).then(res => {
