        ...
        <el-container>
            <el-header>
                ...
                <div>
                    <el-row :gutter="30">
                        <el-col :span="7">
                            <el-input placeholder="名称" v-model="queryParams.itemName" @clear="query" clearable
                                size="large">
                                <template #append>
                ...
                <el-row :gutter="20">
                    <el-col :span="9">
                        <div class="table-main">
                            ...
                            <!-- 表格 -->
                            <el-table :data="customerInfoList" highlight-current-row
                                @current-change="handleChangeCustomer" size="small" style="width: 100% ;color:black;"
                                stripe>
                                <!-- 自动显示序号-->
                                ...
                                <el-table-column align="center" prop="customerSex" label="性别">
                                    <template #default="scope">{{ scope.row.customerSex == 0 ? '男' : '女'
                                        }}</template>
                                </el-table-column>
...
import { listNurseRecordsVo, del } from "@/api/nurseRecordApi.js";
export default {
    computed: {
        // 分页序号不重复 -客户
        indexMethod() {
        ...
        indexMethodRecord() {
            // 计算护理记录表格的起始序号
            return this.pageRecord.currentPage * this.pageRecord.pageSize - this.pageRecord.pageSize + 1;
        },
    },
    data() {
        return {
            ...
            customerInfoList: [],
            nurseRecordList: [], // 护理记录列表数据，需结合实际接口赋值
            page: {
                total: 0,
                pageSize: 6,
                currentPage: 1,
    ...
    methods: {
        ...
        listCustomerInfoPage() {
            listCustomerInfoPage(this.condition).then(res => {
                ...
                this.customerInfoList = res.data.records;
        ...
        listNurseRecordsVo() {
            listNurseRecordsVo(this.conditionRecord).then(res => {
                ...
                this.pageRecord.pageCount = res.data.pages; // 总页数
            });
        },
        // 分页方法
        handleCurrentChange(curPage) {
            this.page.currentPage = curPage;
            this.condition.pageSize = curPage; //参数pagesize是服务器接收页码参数名
            //重新渲染表格
            this.listCustomerInfoPage();
            //清空步骤记录数据表
            this.nurseRecordList = [];
        ...
        del(id) {
            this.$confirm("确定删除？", "提示", {
                ...
                .then(() => {
                    // 调用删除接口，假设 removeCustomerRecord 是封装好的接口方法
