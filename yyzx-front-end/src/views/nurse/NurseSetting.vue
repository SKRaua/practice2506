  ...
  <div class="common-layout">
    <el-container>
      ...
      <el-main>
        ...
        <!-- 护理级别设置抽屉 -->
        <el-drawer title="客户护理设置" :before-close="handleClose" v-model="drawer.openFlag" direction="rtl" size="70%">
          <el-container>
            <el-header>
              <div>
                <el-row>
                  <el-col :span="8">
                    <el-select v-model="drawer.levelId" @change="listNurseItemByLevel" style="width:320px"
                      placeholder="选择护理级别">
                      <el-option v-for="item in drawer.listLevel" :key="item.id" :label="item.levelName"
                        :value="item.id" />
                    </el-select>
                  </el-col>
                </el-row>
...
import { addItemToCustomer, delLevelAndItem } from "@/api/customerNurseItemApi.js";
  ...
  methods: {
    ...
    del(row) {
      this.$confirm("此操作将会移除当前护理级别正在进行中的护理项目?", "提示", {
        ...
        .then(() => {
          delLevelAndItem({ levelId: row.levelId, customerId: row.id }).then(res => {
            if (res.flag) {
              this.$message.success(res.message);
              //重载表格:获取用户列表
              this.listCustomerInfoPage();
